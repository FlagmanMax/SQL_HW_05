...
(
	...
	JOIN
        ...
        ON p.user_id = u.id
    WHERE 
		p.birthday > '2003.05.10'
);

# Вариант 2, можно воспользоваться  функцией DATE_SUB
CREATE OR REPLACE VIEW v_users AS
(
	SELECT
        CONCAT (firstname,' ',lastname) as Username,
	...
	JOIN
        ...
        ON p.user_id = u.id
    WHERE 
		p.birthday >= DATE_SUB(now(),INTERVAL 20 YEAR)
);

# Вариант 3, использую функцию TIMESTAMPDIFF с параметром YEAR
CREATE OR REPLACE VIEW v_users AS
(
	SELECT
        CONCAT (firstname,' ',lastname) as Username,
	...
	JOIN
        ...
        ON p.user_id = u.id
    WHERE 
        (TIMESTAMPDIFF(YEAR,p.birthday,now())) <= 20
);
...
CREATE OR REPLACE VIEW v_um AS
(
SELECT
    DISTINCT(u.id),
    CONCAT(u.firstname,' ',u.lastname) as Username,
    COUNT(u.id) OVER(PARTITION BY id) as MessageCounter
FROM
	users u
JOIN 
	messages m
...
SELECT 
	Username,
    MessageCounter,
    DENSE_RANK() OVER(ORDER BY MessageCounter DESC) Rnk
FROM v_um;
...
SELECT 
	body,
    created_at,
    LAG(created_at,1,0) OVER w AS prev_at,
    TIMESTAMPDIFF(SECOND, LAG(created_at,1,0) OVER w,created_at) as 'Delta_seconds',
    TIMESTAMPDIFF(DAY, LAG(created_at,1,0) OVER w,created_at) as 'Delta_days'
FROM
	messages m
